...
from app.keyboards.calendar import confirm_booking_kb, format_date, month_calendar_kb, slots_kb
...
async def pick_time(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if not date_str or not time_str:
        await callback.answer("Пустые данные слота", show_alert=True)
        return
...
@router.callback_query(StateFilter(BookingStates.waiting_for_phone), F.data == "confirm_booking")
